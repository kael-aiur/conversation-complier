...
public class CompileRunSchemaMigration {
    ...
    public void migrate() {
        ...
        jdbc.execute("CREATE TABLE IF NOT EXISTS knowledge_compile_setting_models (id INTEGER PRIMARY KEY AUTOINCREMENT, setting_id INTEGER NOT NULL DEFAULT 1, provider_id TEXT NOT NULL, model_name TEXT NOT NULL, selection_order INTEGER NOT NULL, FOREIGN KEY (setting_id) REFERENCES knowledge_compile_settings(id) ON DELETE CASCADE, FOREIGN KEY (provider_id) REFERENCES model_providers(id) ON DELETE CASCADE, UNIQUE(setting_id, provider_id, model_name))");
        jdbc.execute("CREATE INDEX IF NOT EXISTS idx_knowledge_compile_setting_models_order ON knowledge_compile_setting_models(setting_id, selection_order)");
        // Migrate an existing single-model setting into the ordered selection table once.
        jdbc.update("INSERT OR IGNORE INTO knowledge_compile_setting_models(setting_id,provider_id,model_name,selection_order) SELECT 1,provider_id,model_name,0 FROM knowledge_compile_settings WHERE id=1 AND provider_id IS NOT NULL AND model_name IS NOT NULL");
