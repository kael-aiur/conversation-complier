...
/** Redacts common credential formats before messages are persisted or exposed. */
public final class FailureMessageSanitizer {
    ...
    private static final Pattern HEADER_SECRET = Pattern.compile(
            "(?i)\\b(x-api-key|x-mcp-key|cf-access-client-id|cf-access-client-secret|api[_-]?key|access[_-]?token|token|secret|password|passwd|private[_-]?key)\\b(\\s*[:=]\\s*)([^\\s,;]+)");
    private static final Pattern JSON_SECRET = Pattern.compile(
            "(?i)([\\\"'](?:x-api-key|x-mcp-key|api[_-]?key|access[_-]?token|token|secret|password|passwd|private[_-]?key|client[_-]?secret)[\\\"']\\s*:\\s*[\\\"']?)([^\\\"'\\s,;}]+)");
    ...
    public static String sanitize(String message) {
        ...
        value = JSON_SECRET.matcher(value).replaceAll("$1[redacted]");
