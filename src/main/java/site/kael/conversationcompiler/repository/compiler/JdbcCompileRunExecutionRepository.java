...
public class JdbcCompileRunExecutionRepository implements CompileRunExecutionRepository {
 ...
 public long startAttempt(long runId,String providerId,String modelName){String now=Instant.now().toString();jdbc.update("INSERT INTO compile_run_attempts(compile_run_id,attempt_number,provider_id,model_name,status,started_at) VALUES(?,(SELECT COALESCE(MAX(attempt_number),0)+1 FROM compile_run_attempts WHERE compile_run_id=?),?,? ,'running',?)",runId,runId,providerId,modelName,now);return jdbc.queryForObject("SELECT last_insert_rowid()",Long.class);}
 public void finishAttempt(long attemptId,String status,String errorType,String errorMessage){jdbc.update("UPDATE compile_run_attempts SET status=?,error_type=?,error_message=?,finished_at=? WHERE id=?",status,errorType,FailureMessageSanitizer.sanitize(errorMessage),Instant.now().toString(),attemptId);}
 ...
 public boolean markCompleted(long id,CompileResultRequest result){String now=Instant.now().toString(); return jdbc.update("UPDATE compile_runs SET status=?, phase='completed', progress=100, summary=?, knowledge_count=(SELECT COUNT(*) FROM compile_run_knowledge_items WHERE compile_run_id=?), result_json=?, finished_at=?, updated_at=? WHERE id=? AND status='running'",result.warnings()==null||result.warnings().isEmpty()?"completed":"completed_with_warnings",result.summary(),id,json(result),now,now,id)==1;}
 public void markFailed(long id,String error){jdbc.update("UPDATE compile_runs SET status='failed', phase='failed', error_message=?, finished_at=?, updated_at=? WHERE id=? AND status='running'",FailureMessageSanitizer.sanitize(error),Instant.now().toString(),Instant.now().toString(),id);}
