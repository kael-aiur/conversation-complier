...
public class JdbcCompileRunExecutionRepository implements CompileRunExecutionRepository {
 ...
 public void markCompleted(long id,CompileResultRequest result){String now=Instant.now().toString();jdbc.update("UPDATE compile_runs SET status=?, phase='completed', progress=100, summary=?, knowledge_count=?, result_json=?, finished_at=?, updated_at=? WHERE id=?",result.warnings()==null||result.warnings().isEmpty()?"completed":"completed_with_warnings",result.summary(),result.items().size(),json(result),now,now,id);}
